...
function Update() {
  form.put(route('Countries.Update'), {
    onSuccess: () => toast.success('تم تحديث البلد  بنجاح'),
...
<template>
  <Head title="تعديل البلد" />

  <div class="w-full py-10 px-10 bg-white" dir="rtl">
    <h1 class="text-3xl font-bold text-right mb-6">تعديل البلد</h1>
    <hr />
    <br />
    <form @submit.prevent="Update">
      ...
      <div class="mt-10 text-left">
        <Link :href="route('Countries.Index')" class="px-8 py-3 text-white font-semibold rounded-lg bg-yellow-600 hover:bg-yellow-700 transition duration-200">
        ...
        <button type="submit" class="px-8 py-3 text-white font-semibold rounded-lg bg-blue-600 hover:bg-blue-700 transition duration-200" :disabled="form.processing">
          {{ form.processing ? '...جاري التحديث' : 'تحديث البلد' }}
        </button>
      </div>
    </form>
  </div>
