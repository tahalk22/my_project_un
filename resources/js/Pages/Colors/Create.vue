...
function Store() {
  form.post(route('Colors.Store'), {
    onSuccess: () => toast.success('تمت إضافة اللون بنجاح'),
...
<template>
  <Head title="إضافة لون" />

  <div class="w-full py-10 px-10 bg-white" dir="rtl">
    <h1 class="text-3xl font-bold text-right mb-6">إضافة لون جديد</h1>
    <hr />
    <br />
    <form @submit.prevent="Store">
      ...
      <div class="mt-10 text-left">
        <button type="submit" class="px-8 py-3 text-white font-semibold rounded-lg bg-blue-600 hover:bg-blue-700 transition duration-200" :disabled="form.processing">
          {{ form.processing ? '...جاري الحفظ' : 'حفظ اللون' }}
        </button>
      </div>
    </form>
  </div>
