...
function ApplyFilters() {
  router.get(route('Colors.Index'), form.value, {
    preserveState: true,
    replace: true,
  });
}
...
function DeleteColor(color) {
  swal.fire({
    title: `حذف اللون؟`,
    html: `سيتم حذف اللون <strong class=\"text-blue-500\">${color.name}</strong> نهائيًا.<br>هل أنت متأكد؟`,
    icon: 'warning',
    ...
    preConfirm: () => {
      router.delete(route('Colors.Delete', { id: color.id }), {
        preserveState: true,
        onSuccess: () => toast.success('تم حذف اللون بنجاح'),
...
<template>
  <Head title="عرض الألوان" />
  <div class="mx-auto px-4 py-8 w-full">
    <h1 class="text-3xl font-bold text-blue-800 mb-6 pb-3 border-b border-gray-200">عرض الألوان</h1>
    ...
    <div class="bg-white rounded-xl shadow-md overflow-hidden border border-gray-200">
        ...
        <table class="min-w-full divide-y divide-gray-200">
          <thead class="bg-blue-800">
            <tr class="text-right text-white text-xs font-medium">
              <th class="px-6 py-3">اسم اللون</th>
              <th class="px-6 py-3">العمليات</th>
            </tr>
          </thead>
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="(color, index) in colors.data" :key="color.id" class="hover:bg-gray-50">
                ...
                <div class="relative inline-block text-left pl-[40px]">
                  ...
                  <div v-if="open_index === index" class="absolute z-10 right-0 mt-2 w-40 bg-white rounded-md shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                    <ul class="py-1 text-sm text-gray-700">
                      <li>
                        <Link :href="route('Colors.Edit', { id: color.id })">
                      ...
                      <li>
                        <button @click="DeleteColor(color)" class="w-full text-right px-4 py-2 hover:bg-gray-100 text-red-600">
      ...
      <Pagination :links="colors.links" :from="colors.from" :to="colors.to" :total="colors.total" :currentPage="colors.current_page" :lastPage="colors.last_page" />
    </div>
  </div>
</template>